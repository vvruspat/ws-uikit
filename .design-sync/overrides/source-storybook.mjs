...
export async function resolveStorybook(ctx) {
  ...
  // When the package's declared types entry exports no PascalCase symbols
  // (e.g. "types" points at a global ambient declarations file rather than the
  // component barrel), the main build's exportedSet is empty and ALL storybook
  // components are dropped. Populate exported from the story component list so
  // the build can proceed — the stories ARE the public surface for this repo.
  const exported = (!ctx.exportedSet || ctx.exportedSet.size === 0)
    ? new Set(csfComponents.map((c) => c.name))
    : undefined;
  return { shape: 'storybook', entry, components: csfComponents, sbStatic, sbDir, exported };
}
